...
|---|---|---|---|---|
| `EVOMTRS_SITE_URL` | Drives canonical URLs, Open Graph URLs, sitemap, robots, and smoke checks. | Needs owner value. Use the final GitHub Pages/custom domain URL, with no trailing slash. Historical reports/example values referenced the hidconsult subdomain; active docs target `https://evomtrs.com`, but final production dispatch domain still needs owner approval. | What exact public production URL should the site use? | Prevents search/social previews and sitemap from pointing at the wrong domain. |
